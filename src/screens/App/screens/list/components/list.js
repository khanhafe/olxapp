...
var AddList = React.createClass({
  ...
  render: function() {
    if (this.refs.Search && this.state.searchTerm != "") {
      ...
      this.state.page.Message = this.state.data.Message.filter(
        this.refs.Search.filter(filters)
      );
    }
    return (
            ...
            return (
              <li>
                <div>
                  <p>
                    <a
                      href={window.location.pathname + "#/" + item.classifiedID}
                    >
                      {item.name}
                    </a>
                  </p>
